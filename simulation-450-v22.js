...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    ...
    if (periodStrategy.borrow) {
        ...
        if (loanAmount > 0 && state.equity > 0) {
            ...
            row++;
            if (debug) console.log(`借入: ¥${loanAmount} → 受取¥${loanAmount - Math.round(loanAmount * 0.1)}, 借入残高¥${state.loans}`);
        }
    }
    ...
    if (period >= 3 && periodStrategy.largeMachine && state.cash >= 100 && state.machinesLarge === 0) {
        ...
        state.cash -= 100;
        row++;
        if (debug) console.log(`★大型機械購入: -¥100 → 現金¥${state.cash}`);
    }
    ...
    // 3期以降: 特急チップ購入（現金を確認しながら）
    if (period >= 3) {
        const targetExpress = periodStrategy.expressResearch || 0;
        for (let i = 0; i < targetExpress && state.cash >= RULES.CHIP.express + 30 && row <= maxRows - 4; i++) {
            // 製造・販売用に現金を残す（最低30円）
            state.research++;
        ...
        if (debug && state.chipsBoughtExpress > 0) console.log(`特急チップ: ${state.chipsBoughtExpress}枚購入`);
    }
    ...
    if (debug) console.log(`期末: 現金¥${state.cash}, 自己資本¥${state.equity}, 次期繰越R${state.nextResearch || 0}`);
...
for (let r2 = 1; r2 <= 3; r2++) {
    for (let nextR = 1; nextR <= 3; nextR++) {
        // 大型機械なし
        STRATEGIES[`R${r2}_N${nextR}_SMALL`] = {
            period2: { research: r2, nextResearch: nextR },
            period3: { nextResearch: nextR },
            period4: { nextResearch: nextR },
            period5: {}
        };

        // 4期に大型機械（自己資本が増えてから）
        for (let s = 1; s <= 2; s++) {
            STRATEGIES[`R${r2}_N${nextR}_L4_S${s}`] = {
                period2: { research: r2, nextResearch: nextR },
                period3: { borrow: 100, worker: 1, salesman: s, nextResearch: nextR },
                period4: { borrow: 150, largeMachine: true, expressResearch: 1, nextResearch: nextR },
        ...
        for (let exp = 0; exp <= 2; exp++) {
            for (let s = 1; s <= 2; s++) {
                STRATEGIES[`R${r2}_N${nextR}_L3_E${exp}_S${s}`] = {
                    period2: { research: r2, nextResearch: nextR },
                    period3: { borrow: 200, largeMachine: true, worker: 1, salesman: s, expressResearch: exp, nextResearch: nextR },
                    period4: { borrow: 100, expressResearch: 1, nextResearch: nextR },
                    period5: { expressResearch: 2 }
                };
            }
        }
...
// 教育チップ込み
STRATEGIES['R2E1_N2_L4_S2'] = {
    period2: { research: 2, education: 1, nextResearch: 2 },
    period3: { borrow: 100, worker: 1, salesman: 2, nextResearch: 2 },
    period4: { borrow: 150, largeMachine: true, expressResearch: 1, nextResearch: 2 },
    period5: { expressResearch: 2 }
};
...
STRATEGIES['STABLE_GROWTH'] = {
    ...
    period3: { borrow: 80, worker: 1, salesman: 1, nextResearch: 2 },
    period4: { borrow: 120, largeMachine: true, salesman: 1, expressResearch: 1, nextResearch: 2 },
    period5: { expressResearch: 2 }
};

console.log('='.repeat(70));
console.log('MG シミュレーション v22 - 保守的投資戦略');
console.log('='.repeat(70));
console.log(`戦略数: ${Object.keys(STRATEGIES).length}`);

// デバッグ
const testName = 'STABLE_GROWTH';
console.log(`\n【${testName}のデバッグ】`);
...
// 追加分析: 最高記録を出した条件
console.log('\n【分析】最高記録を出した条件:');
const maxEquity = Math.max(...Object.values(results).map(r => r.max));
const maxStrategies = Object.entries(results).filter(([_, r]) => r.max >= maxEquity - 10);
maxStrategies.forEach(([name, r]) => {
    console.log(`  ${name}: 最高¥${r.max}, 平均¥${r.avg}`);
});
