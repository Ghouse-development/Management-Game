...
function simulatePeriod2UserPattern(state, strategy, debug = false) {
    const maxRows = 20;
    let row = 1;
    ...
    row++;
    if (debug) console.log(`行${row-1}: PC+保険 → 現金¥${state.cash}`);
    ...
    const targetE = periodStrategy.education || 2;

    // ユーザーパターンを再現
    function buyEducation() {
        if (state.education < targetE && state.cash >= RULES.CHIP.normal && row <= maxRows - 2) {
            state.education++;
            state.cash -= RULES.CHIP.normal;
            state.chipsBoughtNormal++;
            row++;
            if (debug) console.log(`行${row-1}: 教育チップ → 教育${state.education}, 現金¥${state.cash}`);
            return true;
        }
        return false;
    }
    ...
    function buyResearch() {
        if (state.research < targetR && state.cash >= RULES.CHIP.normal && row <= maxRows - 2) {
            ...
            state.chipsBoughtNormal++;
            row++;
            if (debug) console.log(`行${row-1}: 研究チップ → 研究${state.research}, 現金¥${state.cash}`);
            return true;
        }
        return false;
    }

    function buyMaterials(maxQty) {
        if (state.materials < 10 && state.cash >= RULES.MATERIAL_PRICE && row <= maxRows - 2) {
            const qty = Math.min(maxQty, 10 - state.materials, Math.floor(state.cash / RULES.MATERIAL_PRICE));
            if (qty > 0) {
                state.materials += qty;
                state.cash -= qty * RULES.MATERIAL_PRICE;
                totalMaterialCost += qty * RULES.MATERIAL_PRICE;
                row++;
                if (debug) console.log(`行${row-1}: 仕入れ${qty}個 → 材料${state.materials}, 現金¥${state.cash}`);
                return true;
            }
        }
        return false;
    ...
    function process() {
        ...
        if (state.materials > 0 && state.wip < 10 && state.cash >= 1 && row <= maxRows - 2) {
            ...
            if (qty > 0) {
                ...
                state.cash -= qty;
                row++;
                if (debug) console.log(`行${row-1}: 投入${qty}個 → 仕掛${state.wip}`);
            }
        }
        if (state.wip > 0 && state.products < 10 && state.cash >= 1 && row <= maxRows - 2) {
            ...
            if (qty > 0) {
                ...
                state.cash -= qty;
                row++;
                if (debug) console.log(`行${row-1}: 完成${qty}個 → 製品${state.products}`);
            }
        }
    }
    ...
    function sell() {
        ...
        if (state.products > 0 && sc > 0 && row <= maxRows - 2) {
            ...
            row++;
            if (debug) console.log(`行${row-1}: 販売${sellQty}個×¥${price} → 現金¥${state.cash}`);
            return true;
        }
        return false;
    }
    ...
    // 残り行で繰り返し
    while (row <= maxRows - 2) {
        const didSomething = sell() || buyResearch() || buyEducation();
        if (!didSomething) {
            process();
            if (state.products === 0 && state.wip === 0) {
                if (!buyMaterials(5)) break;
            }
        }
    }
    ...
    if (debug) console.log(`期末: 現金¥${state.cash}, 自己資本¥${state.equity}`);
...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    if (period === 2) {
        return simulatePeriod2UserPattern(state, strategy, debug);
    }
    ...
    if (periodStrategy.borrow) {
        ...
        if (loanAmount > 0 && state.equity > 0) {
            ...
            row++;
            if (debug) console.log(`借入: ¥${loanAmount} → 現金¥${state.cash}`);
        }
    }
    ...
    if (periodStrategy.largeMachine && state.cash >= 100 && state.machinesLarge === 0) {
        ...
        state.cash -= 100;
        row++;
        if (debug) console.log(`★大型機械購入 → 現金¥${state.cash}`);
    }
...
for (let exp3 = 0; exp3 <= 2; exp3++) {
    for (let exp4 = 0; exp4 <= 2; exp4++) {
        for (let exp5 = 1; exp5 <= 3; exp5++) {
            for (let s = 1; s <= 2; s++) {
                for (let nextR = 1; nextR <= 2; nextR++) {
                    const name = `R4E1_N${nextR}_L3_E${exp3}${exp4}${exp5}_S${s}`;
                    STRATEGIES[name] = {
...
console.log('MG シミュレーション v28 - ユーザーパターン再現');
console.log('='.repeat(70));
console.log(`戦略数: ${Object.keys(STRATEGIES).length}`);

// デバッグ
const testName = 'R4E2_N2_L3_E123_S2';
console.log(`\n【${testName}のデバッグ】`);
