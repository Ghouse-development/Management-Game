...
function getSalesCapacity(state) {
    ...
    // セールスマン×2 + 広告（セールスマン×2まで有効）×2 + 教育（最大1）
    const baseCapacity = state.salesmen * 2;
    const adBonus = Math.min(state.advertising || 0, baseCapacity) * 2;
    const eduBonus = Math.min(state.education || 0, 1);
    return baseCapacity + adBonus + eduBonus;
}
...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    ...
    if (periodStrategy.borrow) {
        ...
        if (loanAmount > 0 && state.equity > 0) {
            ...
            row++;
            if (debug) console.log(`借入: ¥${loanAmount} → 残高¥${state.loans}`);
        }
    }
    ...
    if (period === 2) {
        ...
        const targetAd = periodStrategy.advertising || 0;
        ...
        while (state.advertising < targetAd && state.cash >= RULES.CHIP.normal && row <= maxRows - 4) {
            state.advertising++;
            state.cash -= RULES.CHIP.normal;
            state.chipsBoughtNormal++;
            row++;
    ...
    if (debug) console.log(`能力: 製造${getMfgCapacity(state)}, 販売${getSalesCapacity(state)}, 研究${state.research}枚, 広告${state.advertising}枚`);
    ...
    while (row <= maxRows - 1) {
        ...
        if (state.materials < 10 && state.cash >= RULES.MATERIAL_PRICE) {
            let maxBuyQty;
            if (period === 2) {
                // 2期は製造能力と無関係に現金の許す限り仕入れ可能
                maxBuyQty = Math.min(10 - state.materials, Math.floor(state.cash / RULES.MATERIAL_PRICE));
            } else {
                // 3期以降は製造能力に依存
                maxBuyQty = Math.min(mc, 10 - state.materials, Math.floor(state.cash / RULES.MATERIAL_PRICE));
            }
            if (maxBuyQty > 0) {
        ...
        for (let exp3 = 0; exp3 <= 2; exp3++) {
            for (let exp4 = 0; exp4 <= 2; exp4++) {
                for (let exp5 = 1; exp5 <= 3; exp5++) {
                    for (let s = 1; s <= 2; s++) {
                        const name = `R4A${ad}_N${nextR}_L3_E${exp3}${exp4}${exp5}_S${s}`;
                        STRATEGIES[name] = {
                            period2: { research: 4, advertising: ad, nextResearch: nextR },
    ...
    for (let nextR = 1; nextR <= 3; nextR++) {
        for (let exp3 = 0; exp3 <= 2; exp3++) {
            for (let exp4 = 0; exp4 <= 2; exp4++) {
                for (let exp5 = 1; exp5 <= 3; exp5++) {
                    for (let s = 1; s <= 2; s++) {
                        const name = `R3A${ad}_N${nextR}_L3_E${exp3}${exp4}${exp5}_S${s}`;
                        STRATEGIES[name] = {
                            period2: { research: 3, advertising: ad, nextResearch: nextR },
                            period3: { borrow: 150, largeMachine: true, worker: 1, salesman: s, expressResearch: exp3, nextResearch: nextR },
                            period4: { borrow: 50, expressResearch: exp4, nextResearch: nextR },
                            period5: { expressResearch: exp5 }
                        };
...
console.log('MG シミュレーション v26 - 広告チップ追加');
console.log('='.repeat(70));
console.log(`戦略数: ${Object.keys(STRATEGIES).length}`);

// デバッグ
const testName = 'R4A2_N2_L3_E023_S2';
console.log(`\n【${testName}のデバッグ】`);
