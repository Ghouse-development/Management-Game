...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    ...
    while (row <= maxRows - 1) {
        ...
        if (state.materials < 10 && state.cash >= RULES.MATERIAL_PRICE) {
            let maxBuyQty;
            if (period === 2) {
                // 2期のみ: 製造能力と無関係に5個まで一気買い可能
                maxBuyQty = Math.min(10 - state.materials, Math.floor(state.cash / RULES.MATERIAL_PRICE), 5);
            } else {
                // 3期以降: 製造能力に依存
...
console.log('MG シミュレーション v25 - 2期のみ仕入れ5個');
console.log('='.repeat(70));
