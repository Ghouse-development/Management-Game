...
function getPrice(researchChips, period) {
    // R0:23, R1:25, R2:27, R3:29, R4:31, R5:32 + 2期は+2
...
function calculateF(state, period, diceRoll) {
    ...
    F += (state.chipsBoughtNormal || 0) * RULES.CHIP.normal;  // 次期繰越用購入
...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    ...
    if (periodStrategy.borrow) {
        ...
        if (loanAmount > 0 && state.equity > 0) {
            ...
            row++;
            if (debug) console.log(`借入: ¥${loanAmount} → 受取¥${loanAmount - Math.round(loanAmount * 0.1)}`);
        }
    }
    ...
    // 3期以降: 次期繰越用チップ購入（優先的に）
    if (period >= 3) {
        const targetNextR = periodStrategy.nextResearch || 0;
        while ((state.nextResearch || 0) < targetNextR && state.cash >= RULES.CHIP.normal && row <= maxRows - 4) {
            state.nextResearch = (state.nextResearch || 0) + 1;
            state.cash -= RULES.CHIP.normal;
            state.chipsBoughtNormal++;
            row++;
            if (debug) console.log(`次期繰越チップ購入: 研究 → 計${state.nextResearch}枚`);
        }
    }
    ...
    if (debug) console.log(`【損益】PQ=${PQ}(${productsSold}個×平均¥${productsSold ? Math.round(PQ/productsSold) : 0}), MQ=${MQ}, F=${F}, G=${G}`);
...
// 高研究・高特急戦略（研究5枚で32円を狙う）
for (let r2 = 2; r2 <= 4; r2++) {          // 2期の研究枚数
    for (let nextR = 2; nextR <= 3; nextR++) { // 次期繰越目標
        for (let exp3 = 1; exp3 <= 3; exp3++) { // 3期特急
            for (let exp4 = 1; exp4 <= 3; exp4++) { // 4期特急
                for (let exp5 = 1; exp5 <= 4; exp5++) { // 5期特急
                    for (let s = 1; s <= 2; s++) { // セールス追加
                        const name = `R${r2}_N${nextR}_E${exp3}${exp4}${exp5}_S${s}`;
                        STRATEGIES[name] = {
                            period2: { research: r2, nextResearch: nextR },
                            period3: { borrow: 200, largeMachine: true, worker: 1, salesman: s,
                                       expressResearch: exp3, nextResearch: nextR },
...
STRATEGIES['ULTRA_R5'] = {
    ...
    period3: { borrow: 200, largeMachine: true, worker: 1, salesman: 2, expressResearch: 3, nextResearch: 3 },
    period4: { borrow: 150, expressResearch: 4, nextResearch: 3 },
    period5: { expressResearch: 5 }
};

console.log('='.repeat(70));
console.log('MG シミュレーション v21 - 高研究・特急チップ戦略');
console.log('='.repeat(70));
console.log(`戦略数: ${Object.keys(STRATEGIES).length}`);

// デバッグ
const testName = 'ULTRA_R5';
console.log(`\n【${testName}のデバッグ】`);
