...
function simulatePeriod(state, period, strategy, diceRoll, debug = false) {
    ...
    if (debug) console.log(`製造${getMfgCapacity(state)}, 販売${getSalesCapacity(state)}, 研究${state.research}枚`);
...
// 2期研究枚数 × 3期大型有無 × セールス数の組み合わせ
for (let r2 = 1; r2 <= 4; r2++) {
    for (let nextR = 1; nextR <= 3; nextR++) {
        // 小型のみ戦略
        STRATEGIES[`R${r2}_N${nextR}`] = {
        ...
        // 3期大型購入戦略（セールス追加でバランス）
        for (let s = 1; s <= 3; s++) {
            STRATEGIES[`R${r2}_N${nextR}_L3_S${s}`] = {
                period2: { research: r2, nextResearch: nextR },
                period3: { borrow: 150, largeMachine: true, worker: 1, salesman: s, nextResearch: nextR, expressResearch: 1 },
                period4: { borrow: 80, nextResearch: nextR, expressResearch: 1 },
...
STRATEGIES['R2E1_N2_L3_S2'] = {
    period2: { research: 2, education: 1, nextResearch: 2 },
    period3: { borrow: 150, largeMachine: true, worker: 1, salesman: 2, nextResearch: 2, expressResearch: 1 },
    period4: { borrow: 80, nextResearch: 2, expressResearch: 1 },
    period5: { expressResearch: 2 }
};

// 高研究戦略
STRATEGIES['R4_N3_L3_S3'] = {
    period2: { research: 4, nextResearch: 3 },
    period3: { borrow: 150, largeMachine: true, worker: 1, salesman: 3, nextResearch: 3, expressResearch: 2 },
    period4: { borrow: 100, nextResearch: 3, expressResearch: 2 },
    period5: { expressResearch: 3 }
};

console.log('='.repeat(70));
console.log('MG シミュレーション v20 - 3期大型機械 + バランス戦略');
console.log('='.repeat(70));
console.log(`戦略数: ${Object.keys(STRATEGIES).length}`);

// デバッグ
const testName = 'R3_N2_L3_S2';
console.log(`\n【${testName}のデバッグ】`);
...
// 10000回シミュレーション
console.log('\n' + '='.repeat(70));
console.log('【10000回シミュレーション TOP20】');
console.log('-'.repeat(70));
...
ranking.forEach(([name, r], i) => {
    ...
    console.log(`${String(i + 1).padStart(2)}. ${name.padEnd(20)} 平均¥${String(r.avg).padStart(4)} 達成${rate.padStart(5)}% P90¥${String(r.p90).padStart(4)} 最高¥${r.max}`);
});
